...
function renderResultViewPage(result, messageSceneId) {
	...
	var resultData = {
		...
		color:color,
		flag:flag,
        poolItemName: result.poolName ? (result.poolName + '-' + result.poolItemName) : '',
	...
	layer_show(result.messageInfo + ' - 测试结果', html, 700, 600 , 1, function(layero, index) {
        ...
        layero.find("#view-proxy-info").click(function(){
            if (strIsNotEmpty(result.proxyInfo)) {
                var proxyInfo = JSON.parse(result.proxyInfo)
                if (proxyInfo.useProxy == "是") {
                    layer_show('代理信息', templates["scene-test-result-proxy"](proxyInfo), 380, 220, 1);
                    return;
                }
            }
            layer.msg("没有使用代理", {icon:0, time:1500});
            return;
        });
...
/**
 * 获取数据源ID
 * @param callback 回调方法,两个参数，分别为dbId,dbName
 */
    ...
    $.ajax({
        ...
        success:function(data) {
            if(data.returnCode == 0){
                ...
                var selectHtml = '<div class="row cl" style="width:340px;margin:15px;"><div class="form-label col-xs-2"><input type="button" id="select-db-btn" class="btn btn-primary radius" value="选择"/></div><div class="formControls col-xs-10"><span class="select-box radius mt-0"><select class="select" size="1" name="selectDb" id="selectDb">';
                $.each(data.data, function(i,n) {
                    selectHtml += '<option value="' + n.dbId + '">' + n.dbUrl + "-" + n.dbName + '</option>';
                });
                selectHtml += '</select></span></div></div>';
                let index_selectDb = layer.open({
                    type: 1,
                    title: "选择数据库",
                    offset: '30px',
                    area: ['355px', '110px'],
                    content:selectHtml,
                    success:function(layero, index) {
                        $(layero).find('#select-db-btn').click(function(){
                            if (typeof callback === 'function') {
                                callback($("#selectDb").val(), $("#selectDb option:selected").text());
                            }
