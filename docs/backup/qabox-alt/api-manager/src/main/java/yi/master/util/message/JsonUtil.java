...
public class JsonUtil {
	...
	public static Object getJsonList(String jsonStr, int mode) throws Exception {
		...
		Object obj = new ObjectMapper().readValue(jsonStr, Object.class);

        viewJsonTree(obj, jsonTreeMap, jsonTreeList, jsonTreeType,
	...
	public static String getObjectByJson(String jsonStr, String argsPath,
		...
		try {
			...
			if (argsPath.indexOf(".") >= 0) {
				// 类型自适应
				obj = getObject(maps, argsPath, argsType, i, argsPath.split("\\.")[i]);
			} else { // 第一层获取
	 ...
	 * @return
	 */
	public static Object getObject(Object m, String key, TypeEnum type, int count, String thisKey) {
		if (m == null) {
			return null;
		}
		Object o = null;
		...
		try {
					...
					if (count < key.split("\\.").length
						...
						if (e.getValue() instanceof String
							...
							if (count == key.split("\\.").length) {
								...
								o = getObject(e.getValue(), key, type, count, key.split("\\.")[count]);
							}
							return o;
						} else if (e.getValue() instanceof ArrayList) {
							if (count == key.split("\\.").length) {
			...
			if (m instanceof List || m instanceof ArrayList) {
				ls = (ArrayList) m;
				String newKey = key.split("\\.")[count];
				String sub = ReUtil.get("\\[\\d+\\]", newKey, 0);
				int listNum = 0;
				if (StringUtils.isNotBlank(sub)) {
                    sub = ReUtil.get("\\d+", sub, 0);
                    newKey = newKey.replace("[" + sub + "]", "");
				...
				for (int i = 0; i < ls.size(); i++) {
					...
					if (ls.get(i) instanceof LinkedHashMap) {
						if (i == key.split("\\.").length) {
							...
							o = getObject(ls.get(i), key, type, count, newKey);
						}
						return o;
