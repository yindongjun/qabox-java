...
public class ImportInterfaceInfo {
	 ...
	 * @return
	 */
	public static Map<String, Object> importToDB (String path, Integer projectId) {
		if (projectId == null) {
			projectId = SystemConsts.DefaultObjectId.DEFAULT_PROJECT_ID.getId();
		}
		...
		for (InterfaceInfo info:infos) {
			...
			try {
				...
				info.setProjectInfo(new ProjectInfo(projectId));
				...
				if (StringUtils.isNotBlank(info.getCreateMessage())) {
					...
					message.setStatus("0");
					message.setProjectInfo(new ProjectInfo(projectId));
				}
				...
				if (message != null && StringUtils.isNotBlank(info.getCreateScene())) {
					...
					scene.setMark("");
					scene.setProjectInfo(new ProjectInfo(projectId));
				}
	...
	public static Object[] validateInfo (InterfaceInfo info) {
		...
		try {
			...
			if (!MessageKeys.InterfaceBusiType.SL.name().equalsIgnoreCase(info.getInterfaceType()) && !MessageKeys.InterfaceBusiType.CX.name().equalsIgnoreCase(info.getInterfaceType())) {
				info.setInterfaceType(MessageKeys.InterfaceBusiType.CX.name());
