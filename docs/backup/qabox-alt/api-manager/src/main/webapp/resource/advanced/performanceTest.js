...
var eventList = {		
						...
						layer.confirm('初始化完成,是否需要打开测试视图页面？', {title:'提示'}, function(index){
							$.post(REQUEST_URL.PERFORMANCE_TEST.TASK_VIEW, {objectId:json.data.objectId}, function(text){
								...
								if (text.returnCode == 0) {	
									layer_show("性能测试视图", templates["performance-test-task-view"](text.data), null, null, 1, function(layero, index){
										createTestView(layero, text.data);
									}, null, function(){
		...
		"#start-pt-test":function(){
			...
			if (currentTestObject.finished) {
                layer.msg('测试已结束！', {time:1600});
                return;
			}
		...
		"#stop-pt-test":function(){
            if (!currentTestObject.running) {
                layer.msg('测试任务不是运行状态！', {time:1600});
                return;
            }
            if (currentTestObject.finished) {
                layer.msg('测试已结束！', {time:1600});
			...
			$.post(REQUEST_URL.PERFORMANCE_TEST.TASK_VIEW, {objectId:currentTestObject.objectId}, function(json){
				if (json.returnCode == 0) {
					if (!json.data.running) {
						clearInterval(window.intervalId);
                        $(layero).find('.col-sm-2:eq(2) .label-success').text("已结束");
                        currentTestObject.finished = true;
                        currentTestObject.running = false;
						return;
					}
