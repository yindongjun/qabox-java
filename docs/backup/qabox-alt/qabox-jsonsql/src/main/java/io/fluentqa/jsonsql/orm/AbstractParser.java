...
import io.fluentqa.jsonsql.JSON;
import io.fluentqa.jsonsql.JSONRequest;
import io.fluentqa.jsonsql.JSONResponse;
import io.fluentqa.jsonsql.Log;
import io.fluentqa.jsonsql.NotNull;
import io.fluentqa.jsonsql.RequestMethod;
import io.fluentqa.jsonsql.StringUtil;
import io.fluentqa.jsonsql.orm.exception.CommonException;
import io.fluentqa.jsonsql.*;
...
public abstract class AbstractParser<T extends Object> implements Parser<T>, ParserCreator<T>, VerifierCreator<T>, SQLCreator {
	...
	public JSONObject parseResponse(JSONObject request) {
		...
		if (isNeedVerifyRole() && globalRole == null) {
			try {
				setGlobalRole(requestObject.getString(io.fluentqa.jsonsql.JSONRequest.KEY_ROLE));
				requestObject.remove(io.fluentqa.jsonsql.JSONRequest.KEY_ROLE);
			} catch (Exception e) {
				return extendErrorResult(requestObject, e, requestMethod, getRequestURL(), isRoot);
			}
		}
		...
		try {
			setGlobalFormat(requestObject.getBoolean(io.fluentqa.jsonsql.JSONRequest.KEY_FORMAT));
			setGlobalDatabase(requestObject.getString(io.fluentqa.jsonsql.JSONRequest.KEY_DATABASE));
			setGlobalSchema(requestObject.getString(io.fluentqa.jsonsql.JSONRequest.KEY_SCHEMA));
			setGlobalDatasource(requestObject.getString(io.fluentqa.jsonsql.JSONRequest.KEY_DATASOURCE));
			setGlobalExplain(requestObject.getBoolean(io.fluentqa.jsonsql.JSONRequest.KEY_EXPLAIN));
			setGlobalCache(requestObject.getString(io.fluentqa.jsonsql.JSONRequest.KEY_CACHE));

			requestObject.remove(io.fluentqa.jsonsql.JSONRequest.KEY_FORMAT);
			requestObject.remove(io.fluentqa.jsonsql.JSONRequest.KEY_DATABASE);
			requestObject.remove(io.fluentqa.jsonsql.JSONRequest.KEY_SCHEMA);
			requestObject.remove(io.fluentqa.jsonsql.JSONRequest.KEY_DATASOURCE);
			requestObject.remove(io.fluentqa.jsonsql.JSONRequest.KEY_EXPLAIN);
			requestObject.remove(io.fluentqa.jsonsql.JSONRequest.KEY_CACHE);
		} catch (Exception e) {
			return extendErrorResult(requestObject, e, requestMethod, getRequestURL(), isRoot);
		}
	...
	public static JSONObject wrapRequest(RequestMethod method, String tag, JSONObject object, boolean isStructure) {
		...
		if (object == null || object.containsKey(tag)) { //tag 是 Table 名或 Table[]
			if (putTag) {
				...
				object.put(io.fluentqa.jsonsql.JSONRequest.KEY_TAG, tag);
			}
			return object;
		}

		boolean isDiffArrayKey = tag.endsWith(":[]");
		boolean isArrayKey = isDiffArrayKey || io.fluentqa.jsonsql.JSONRequest.isArrayKey(tag);
		...
		if (putTag) {
			target.put(io.fluentqa.jsonsql.JSONRequest.KEY_TAG, tag);
		}
	...
	public JSONObject parseCorrectRequest() throws Exception {
		setTag(requestObject.getString(io.fluentqa.jsonsql.JSONRequest.KEY_TAG));
		setVersion(requestObject.getIntValue(io.fluentqa.jsonsql.JSONRequest.KEY_VERSION));
		requestObject.remove(io.fluentqa.jsonsql.JSONRequest.KEY_TAG);
		requestObject.remove(io.fluentqa.jsonsql.JSONRequest.KEY_VERSION);
	...
	public JSONObject getStructure(@NotNull String table, String method, String tag, int version) throws Exception  {
		...
		if (result == null) {
			...
			//获取指定的JSON结构 <<<<<<<<<<<<<<
			SQLConfig config = createSQLConfig().setMethod(RequestMethod.GET).setTable(table);
			config.setPrepared(false);
			config.setColumn(Arrays.asList("structure"));

			Map<String, Object> where = new HashMap<String, Object>();
			where.put("method", method);
			where.put(io.fluentqa.jsonsql.JSONRequest.KEY_TAG, tag);

			if (version > 0) {
				where.put(io.fluentqa.jsonsql.JSONRequest.KEY_VERSION + ">=", version);
			}
			config.setWhere(where);
			config.setOrder(io.fluentqa.jsonsql.JSONRequest.KEY_VERSION + (version > 0 ? "+" : "-"));
			config.setCount(1);
   ...
   */
	...
	public JSONObject onObjectParse(final JSONObject request
		...
		io.fluentqa.jsonsql.orm.Entry<String, String> entry = Pair.parseEntry(name, true);
		...
		if (op != null) {//SQL查询结果为空时，functionMap和customMap没有意义
			...
			if (arrayConfig == null) { //Common
				...
				if (type == SQLConfig.TYPE_ITEM_CHILD_0 && query != io.fluentqa.jsonsql.JSONRequest.QUERY_TABLE && position == 0) {
					...
					if (rp != null) {
						...
						if (index >= 0) {
							...
							pagination.put(io.fluentqa.jsonsql.JSONRequest.KEY_COUNT, count);
							pagination.put(io.fluentqa.jsonsql.JSONRequest.KEY_PAGE, page);
							...
							if (total <= count*page) {
								query = io.fluentqa.jsonsql.JSONRequest.QUERY_TOTAL;//数量不够了，不再往后查询
							}
						}
					}
				...
				//Table
				if (query == io.fluentqa.jsonsql.JSONRequest.QUERY_TOTAL) {
	...
	public JSONArray onArrayParse(JSONObject request, String parentPath, String name, boolean isSubquery) throws Exception {
		...
		final String query = request.getString(io.fluentqa.jsonsql.JSONRequest.KEY_QUERY);
		final Boolean compat = request.getBoolean(io.fluentqa.jsonsql.JSONRequest.KEY_COMPAT);
		final Integer count = request.getInteger(io.fluentqa.jsonsql.JSONRequest.KEY_COUNT); //TODO 如果不想用默认数量可以改成 getIntValue(JSONRequest.KEY_COUNT);
		final Integer page = request.getInteger(io.fluentqa.jsonsql.JSONRequest.KEY_PAGE);
		final Object join = request.get(io.fluentqa.jsonsql.JSONRequest.KEY_JOIN);

		int query2;
		if (query == null) {
			query2 = io.fluentqa.jsonsql.JSONRequest.QUERY_TABLE;
		}
		else {
			switch (query) {
			case "0":
			case io.fluentqa.jsonsql.JSONRequest.QUERY_TABLE_STRING:
				query2 = io.fluentqa.jsonsql.JSONRequest.QUERY_TABLE;
				break;
			case "1":
			case io.fluentqa.jsonsql.JSONRequest.QUERY_TOTAL_STRING:
				query2 = io.fluentqa.jsonsql.JSONRequest.QUERY_TOTAL;
				break;
			case "2":
			case io.fluentqa.jsonsql.JSONRequest.QUERY_ALL_STRING:
				query2 = io.fluentqa.jsonsql.JSONRequest.QUERY_ALL;
				break;
			default:
				throw new IllegalArgumentException(path + "/" + io.fluentqa.jsonsql.JSONRequest.KEY_QUERY + ":value 中 value 的值不合法！必须在 [0,1,2] 或 [TABLE, TOTAL, ALL] 内 !");
			}
		}
		...
		if (page2 < 0 || page2 > maxPage) {
			throw new IllegalArgumentException(path + "/" + io.fluentqa.jsonsql.JSONRequest.KEY_PAGE + ":value 中 value 的值不合法！必须在 0-" + maxPage + " 内 !");
		}
		...
		if (count2 < 0 || count2 > max) {
			throw new IllegalArgumentException(path + "/" + io.fluentqa.jsonsql.JSONRequest.KEY_COUNT + ":value 中 value 的值不合法！必须在 0-" + max + " 内 !");
		}

		request.remove(io.fluentqa.jsonsql.JSONRequest.KEY_QUERY);
		request.remove(io.fluentqa.jsonsql.JSONRequest.KEY_COMPAT);
		request.remove(io.fluentqa.jsonsql.JSONRequest.KEY_COUNT);
		request.remove(io.fluentqa.jsonsql.JSONRequest.KEY_PAGE);
		request.remove(io.fluentqa.jsonsql.JSONRequest.KEY_JOIN);
		...
		try {
			...
			else if (childKeys.length == 1 && io.fluentqa.jsonsql.JSONRequest.isTableKey(childKeys[0])) {  // 可能无需提取，直接返回 rawList 即可
			...
			//后面还可能用到，要还原
			request.put(io.fluentqa.jsonsql.JSONRequest.KEY_QUERY, query);
			request.put(io.fluentqa.jsonsql.JSONRequest.KEY_COMPAT, compat);
			request.put(io.fluentqa.jsonsql.JSONRequest.KEY_COUNT, count);
			request.put(io.fluentqa.jsonsql.JSONRequest.KEY_PAGE, page);
			request.put(io.fluentqa.jsonsql.JSONRequest.KEY_JOIN, join);
		}
	...
	static {  // TODO 不全
		...
		JOIN_COPY_KEY_LIST.add(io.fluentqa.jsonsql.JSONRequest.KEY_ROLE);
		JOIN_COPY_KEY_LIST.add(io.fluentqa.jsonsql.JSONRequest.KEY_DATABASE);
		JOIN_COPY_KEY_LIST.add(io.fluentqa.jsonsql.JSONRequest.KEY_SCHEMA);
		JOIN_COPY_KEY_LIST.add(io.fluentqa.jsonsql.JSONRequest.KEY_DATASOURCE);
		JOIN_COPY_KEY_LIST.add(io.fluentqa.jsonsql.JSONRequest.KEY_COLUMN);
		JOIN_COPY_KEY_LIST.add(io.fluentqa.jsonsql.JSONRequest.KEY_NULL);
		JOIN_COPY_KEY_LIST.add(io.fluentqa.jsonsql.JSONRequest.KEY_CAST);
		JOIN_COPY_KEY_LIST.add(io.fluentqa.jsonsql.JSONRequest.KEY_COMBINE);
		JOIN_COPY_KEY_LIST.add(io.fluentqa.jsonsql.JSONRequest.KEY_GROUP);
		JOIN_COPY_KEY_LIST.add(io.fluentqa.jsonsql.JSONRequest.KEY_HAVING);
		JOIN_COPY_KEY_LIST.add(io.fluentqa.jsonsql.JSONRequest.KEY_HAVING_AND);
		JOIN_COPY_KEY_LIST.add(io.fluentqa.jsonsql.JSONRequest.KEY_ORDER);
		JOIN_COPY_KEY_LIST.add(io.fluentqa.jsonsql.JSONRequest.KEY_RAW);
	}
	...
	private List<Join> onJoinParse(Object join, JSONObject request) throws Exception {
		...
		for (Entry<String, Object> e : set) {  // { &/User:{}, </Moment/id@":{}, @/Comment/toId@:{} }
			...
			if (outer instanceof JSONObject == false) {
				throw new IllegalArgumentException(io.fluentqa.jsonsql.JSONRequest.KEY_JOIN + ":value 中value不合法！"
						+ "必须为 &/Table0/key0,</Table1/key1,... 或 { '&/Table0/key0':{}, '</Table1/key1':{},... } 这种形式！");
			}

			int index = path == null ? -1 : path.indexOf("/");
			if (index < 0) {
				throw new IllegalArgumentException(io.fluentqa.jsonsql.JSONRequest.KEY_JOIN + ":value 中 value 值 " + path + " 不合法！"
      ...
      if (arrKey != null && io.fluentqa.jsonsql.JSONRequest.isArrayKey(arrKey) == false) {
        throw new IllegalArgumentException(io.fluentqa.jsonsql.JSONRequest.KEY_JOIN + ":'" + e.getKey() + "' 对应的 " + arrKey + " 不是合法的数组 key[] ！" +
          "@ APP JOIN 最多允许跨 1 层，只能是子数组，且数组对象中不能有 join: value 键值对！");
      }

      tableKey = index2 < 0 ? tableKey : tableKey.substring(index2+1);

      io.fluentqa.jsonsql.orm.Entry<String, String> entry = Pair.parseEntry(tableKey, true);
      String table = entry.getKey(); // User
			if (StringUtil.isName(table) == false) {
				throw new IllegalArgumentException(io.fluentqa.jsonsql.JSONRequest.KEY_JOIN + ":value 中 value 的 Table 值 " + table + " 不合法！"
			...
			if (StringUtil.isNotEmpty(alias, true) && StringUtil.isName(alias) == false) {
				throw new IllegalArgumentException(io.fluentqa.jsonsql.JSONRequest.KEY_JOIN + ":value 中 value 的 alias 值 " + alias + " 不合法！"
						+ "必须为 &/Table0,</Table1/key1,@/Table1:alias2/key2,... 或 { '&/Table0':{}, '</Table1/key1':{},... } 这种格式！"
        ...
        if (tableObj == null) {
				...
				throw new IllegalArgumentException(io.fluentqa.jsonsql.JSONRequest.KEY_JOIN + ":'" + e.getKey() + "' 对应的 " + tableKey + ":value 中 value 类型不合法！" +
          "必须是 {} 这种 JSONObject 格式！" + e2.getMessage());
			}

      if (arrKey != null) {
        if (parentPathObj.get(io.fluentqa.jsonsql.JSONRequest.KEY_JOIN) != null) {
          throw new IllegalArgumentException(io.fluentqa.jsonsql.JSONRequest.KEY_JOIN + ":'" + e.getKey() + "' 对应的 " + arrKey + ":{ join: value } 中 value 不合法！" +
            "@ APP JOIN 最多允许跨 1 层，只能是子数组，且数组对象中不能有 join: value 键值对！");
        }

        Integer subPage = parentPathObj.getInteger(io.fluentqa.jsonsql.JSONRequest.KEY_PAGE);
        if (subPage != null && subPage != 0) {
          throw new IllegalArgumentException(io.fluentqa.jsonsql.JSONRequest.KEY_JOIN + ":'" + e.getKey() + "' 对应的 " + arrKey + ":{ page: value } 中 value 不合法！" +
      ...
      boolean isAppJoin = "@".equals(joinType);
			...
			if (key != null) {  // 指定某个 key 为 JOIN ON 条件
				if (key.indexOf("@") != key.length() - 1) {
					throw new IllegalArgumentException(io.fluentqa.jsonsql.JSONRequest.KEY_JOIN + ":" + e.getKey() + " 中 " + key + " 不合法！"
							+ "必须为 &/Table0,</Table1/key1,@/Table1:alias2/key2,... 或 { '&/Table0':{}, '</Table1/key1':{},... } 这种格式！"
							+ "且 Table:alias 的 alias 必须满足英文单词变量名格式！");
				}

				if (tableObj.get(key) instanceof String == false) {
					throw new IllegalArgumentException(io.fluentqa.jsonsql.JSONRequest.KEY_JOIN + ":" + e.getKey() + "' 对应的 "
        ...
        if (isAppJoin && StringUtil.isName(key.substring(0, key.length() - 1)) == false) {
          throw new IllegalArgumentException(io.fluentqa.jsonsql.JSONRequest.KEY_JOIN + ":'" + e.getKey() + "' 中 " + key + " 不合法 ！" +
			...
			for (Entry<String, Object> tableEntry : tableSet) {
				...
				if (k.length() > 1 && k.indexOf("@") == k.length() - 1 && v instanceof String) {
					...
					if (ind == 0 && key == null) {  // 指定了某个就只允许一个 ON 条件
						...
						io.fluentqa.jsonsql.orm.Entry<String, String> te = tk == null || p.substring(ind2 + 1).indexOf("/") >= 0 ? null : Pair.parseEntry(tk, true);

						if (te != null && io.fluentqa.jsonsql.JSONRequest.isTableKey(te.getKey()) && request.get(tk) instanceof JSONObject) {
              if (isAppJoin) {
                if (refObj.size() >= 1) {
                  throw new IllegalArgumentException(io.fluentqa.jsonsql.JSONRequest.KEY_JOIN + ":" + e.getKey() + " 中 " + k + " 不合法！"
              ...
              refObj.put(k, v);
					...
					throw new UnsupportedOperationException(table + "/" + k + " 不合法！" + io.fluentqa.jsonsql.JSONRequest.KEY_JOIN + " 关联的 Table 中，"
			...
			if (arrKey != null) {
        Integer count = parentPathObj.getInteger(io.fluentqa.jsonsql.JSONRequest.KEY_COUNT);
        j.setCount(count == null ? getDefaultQueryCount() : count);
			...
			for (Entry<String, Object> refEntry : refSet) {
				...
				// 主表允许别名
				io.fluentqa.jsonsql.orm.Entry<String, String> targetEntry = Pair.parseEntry(targetTableKey, true);
			...
			if (explain) {
				...
				if (RequestMethod.isQueryMethod(config.getMethod())){
					...
					if (explainResult == null) {
						...
						result.put(JSONObject.KEY_EXPLAIN, explainResult);
						result.putAll(res);
					}
				}
