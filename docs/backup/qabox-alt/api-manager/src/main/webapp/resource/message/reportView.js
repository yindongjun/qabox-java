	...
	'tbody > tr':function() {
        ...
        layer.open({
            ...
            success:function(layero) {            	
                ...
                $(".layui-layer-content > .view-details > .row:eq(2) .col-sm-9").text(resultInfo.poolItemName ? resultInfo.poolName + '-' + resultInfo.poolItemName : '未使用');
                $(".layui-layer-content > .view-details > .row:eq(3) .col-sm-9").text(resultInfo.statusCode);
                $(".layui-layer-content > .view-details > .row:eq(4) .col-sm-9 textarea").text(resultInfo.requestMessage);
                $(".layui-layer-content > .view-details > .row:eq(5) .col-sm-9 textarea").text((resultInfo.responseMessage != null) ? resultInfo.responseMessage : "");
                $(".layui-layer-content > .view-details > .row:eq(6) .col-sm-9 textarea").text((resultInfo.mark != null) ? resultInfo.mark : "");
            }
        });
	},
	'.select-view-mark':{
   	 	...
   	 	customCallBack:function(params){
				...
				if (data.returnCode == 0) {
			        ...
			        $.each(data.desc.systems, function(i, name){
			        	$selectSystems.append('<option value="' + name + '">' + (name || '组合场景') + '</option>');
			        });
    ...
    $.each(resultData, function(i, report){
    	...
    	if ((report.runStatus == publicStatus || publicStatus == "all")
            ...
            dataHtml += '<tr id="' + i + '">'    
                    ...
                    + '<td>' + (report.poolItemName ? report.poolName + '-' + report.poolItemName : '未使用') + '</td>'
                    + '<td class="status"><span>' + report.runStatus + '</span></td>'
                    + '<td>' + report.useTime + '</td>'
                    + '</tr>';
            if (report.protocolType == "FIXED") {
            	dataHtml += '<tr class="hidden fixed-complex-table"><td colspan="9"><table class="table table-bordered">';
