...
public class PerformanceTestObject {
	...
	public PerformanceTestObject(PerformanceTestConfig config, User testUser) {
		...
		this.projectId = config.getProjectInfo().getProjectId();
	...
	public boolean init() {
		...
		if (testConfig == null) {
			testConfig = testConfigService.configByUserId(0);
		}
		testScene = new TestMessageScene(config.getMessageScene(), requestUrl, new String(requestMessage), 0, false, testConfig, PracticalUtils.jsonToMap(PracticalUtils.replaceGlobalVariable(msg.getCallParameter(), null)));
	...
	private boolean end() {
		if (enableSave) {
			...
			try {
				filePath = CustomSettingVariable.PT_RESULT_FILE_SAVE_PATH + PracticalUtils.getUUID("") + ".txt";
	...
	class ThreadPerformanceTest implements Runnable {
		...
		public ThreadPerformanceTest(List<Map<String, String>> threadParameters, int maxCount) {
            ...
            FrameworkUtil.getSessionMap().put(SystemConsts.SESSION_ATTRIBUTE_LOGIN_USER, testUser);
		}
