...
var templateParams = {
		...
		tableTheads:["接口/组合场景", "结果", "状态码", "耗时ms", "测试时间", "备注","详情"]
};
...
var eventList = {
	...
	'.show-result-detail':function() {
		var data = table.row( $(this).parents('tr') ).data();
        if (data.protocolType == 'FIXED') {
            if (data.complexSceneResults == null || data.complexSceneResults.length < 1) {
                layer.msg('该组合场景没有测试结果', {icon: 0, time: 1800});
                return false;
            }
            layer_show( '组合场景-测试结果  点击查看对应详情', templates["complex-scene-results-view"]({results:data.complexSceneResults}), 600, 300, 1, function(layero, index){
                layero.find('.result-view').bind('click', function(){
                    renderResultViewPage(data.complexSceneResults[$(this).attr("data-id")]);
                });
