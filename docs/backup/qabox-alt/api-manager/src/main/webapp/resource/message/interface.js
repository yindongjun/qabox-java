...
var templateParams = {
		tableTheads:["名称", "中文名","类型","协议","创建时间","状态","创建用户","最后修改","报文管理", "入参节点", "备注","操作"],
		btnTools:[{
			type:"primary",
			size:"M",
			id:"add-object",
            ...
            children: [
                ...
                {
                    name: '更新测试环境',
                    id: 'batch-update-system'
                },
                {
                    name: '新增接口参数',
                    id: 'batch-insert-parameter'
                }
            ]
		},{
			type:"success",
...
var eventList = {
        "#batch-insert-parameter":function () {
            layer_show("批量新增参数", htmls['interface-batchAddParameter'], 777, 575, 1, function (layero, index) {
                $(layero).find('#confirm-batch-add').click(function () {
                    let msg = $('#batch-add-param-json').val();
                    if (!strIsNotEmpty(msg)) {
                        layer.msg('请输入完整再提交！', {icon: 0, time:1800});
                        return;
                    }
                    if (!isJSON(msg)) {
                        layer.msg('提交报文格式有误！', {icon: 0, time:1800});
                        return;
                    }
                    loading(true, "正在更新...");
                    $.post(REQUEST_URL.INTERFACE.BATCH_UPDATE_PARAM, JSON.parse(msg), function (json) {
                        loading(false);
                        if (json.returnCode == 0) {
                            layer.msg('批量新增成功', {icon: 1, time:1800})
                            refreshTable();
                        } else {
		...
		"#import-data-from-excel":function() {
			createImportExcelMark("Excel导入接口信息", "../../excel/upload_interface_template.xlsx"
					, REQUEST_URL.FILE.UPLOAD_FILE, REQUEST_URL.INTERFACE.IMPORT_FROM_EXCEL + "?projectId=" + top.currentProjectId);
		},
