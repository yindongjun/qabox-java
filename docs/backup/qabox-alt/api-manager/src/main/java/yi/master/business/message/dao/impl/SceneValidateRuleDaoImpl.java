...
public class SceneValidateRuleDaoImpl extends BaseDaoImpl<SceneValidateRule> implements SceneValidateRuleDao{
	...
	public List<SceneValidateRule> getConfigRules(Integer configId) {
		String hql = "From SceneValidateRule s where s.testConfig.configId=:configId";
		return getSession().createQuery(hql).setInteger("configId", configId).setCacheable(true).list();
	}
