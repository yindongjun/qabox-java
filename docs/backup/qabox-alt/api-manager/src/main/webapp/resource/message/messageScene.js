...
var eventList = {
		...
		".set-complex-scene-setting":function () {//打开设定该组合场景中测试场景的配置
			...
			layer_show("编辑配置信息", templates["scene-show-variables"](data.config), null, null, 1, function () {
				...
				$.each($("#save-variables").siblings('div').children(".edit-this-variables"), function(i, n) {
					...
					updateConfig["saveVariables"][variables.substring(0, variables.lastIndexOf("="))] = variables.substring(variables.lastIndexOf("=") + 1);
				});
				$.each($("#use-variables").siblings('div').children(".edit-this-variables"), function(i, n) {
					var variables = $(n).text();
					updateConfig["useVariables"][variables.substring(0, variables.lastIndexOf("="))] = variables.substring(variables.lastIndexOf("=") + 1);
				});
        ...
        '#choose-scene-variables-key-req-use':function () {//替换变量-入参节点获取
            chooseParameterNodePath(REQUEST_URL.MESSAGE_SCENE.GET_REQUEST_MSG_JSON_TREE, {messageSceneId:messageSceneId}, {
                ...
                isChoosePath:true,
                notChooseTypes:["Map", "List", "Object"],
                choosenCallback:function (path) {
                    $("#scene-variables-key").val(path);
                }
            });
        },
        '#choose-scene-variables-key-req-add':function () {//保存变量-入参节点获取
            chooseParameterNodePath(REQUEST_URL.MESSAGE_SCENE.GET_REQUEST_MSG_JSON_TREE, {messageSceneId:messageSceneId}, {
                titleName:currentScene.interfaceName + "-" + currentScene.messageName + "-" + currentScene.sceneName + " 入参节点树",
                isChoosePath:true,
                notChooseTypes:["Array", "Map", "List", "Object"],
                choosenCallback:function (path) {
                    $("#scene-variables-key").val('RequestMessage.' + path);
                }
            });
        },
        '#choose-scene-variables-key-resp-add':function () {//保存变量-出参节点获取
        ...
        //自定义关联获取组合场景的保存变量
        "#choose-scene-variables-key-relation-add":function() {
		    let v = $('#scene-variables-key').val();
		    if (strIsNotEmpty(v)) {
		        v = v.replace(/"#"/g, '":"');
            }
		    if (!isJSON(v)) {
            ...
            layer_show("编辑关联规则", templates["complex-scene-variable-relation-keyword"](JSON.parse(v)), 890, 315, 1, null
                , function(index, layero) {
                    v = {};
                    v.searchScope = $(layero).find('#searchScope').val();
                    v.LB = $(layero).find('#LB').val();
                    v.RB = $(layero).find('#RB').val();
                    v.OFFSET = $(layero).find('#OFFSET').val();
                    v.ORDER = $(layero).find('#ORDER').val();
                    v.LENGHT = $(layero).find('#LENGHT').val();

                    $('#scene-variables-key').val(JSON.stringify(v).replace(/":"/g, '"#"'));
                    layer.msg('已保存配置!', {icon: 1, time:1600});
                    layer.close(index);
                }, null, {shadeClose: false});
        },
		...
		"#import-data-from-excel":function() {//Excel导入数据
			createImportExcelMark("Excel导入场景信息", "../../excel/upload_scene_template.xlsx"
					, REQUEST_URL.FILE.UPLOAD_FILE, REQUEST_URL.MESSAGE_SCENE.IMPORT_FROM_EXCEL + "?messageId=" + messageId +  + "&projectId=" + top.currentProjectId);
		},
...
function editSceneConfigVariable (obj, editMode) {
	...
	if (editMode == "edit") {
		...
		context = {"key":keyValue.substring(0, keyValue.lastIndexOf("=")), "value":keyValue.substring(keyValue.lastIndexOf("=") + 1)};
	...
	layer_show(title + "-" + name, templates["scene-edit-variables"](context), 875, 260, 1, function(layero, index) {
