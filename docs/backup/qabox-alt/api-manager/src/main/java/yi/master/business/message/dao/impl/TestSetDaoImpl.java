...
public class TestSetDaoImpl extends BaseDaoImpl<TestSet> implements TestSetDao {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<TestSet> getRootSet(Integer projectId) {
		String hql = "from TestSet t where t.parentSet=null and t.projectInfo.projectId=:projectId";
		return getSession().createQuery(hql).setInteger("projectId", projectId).setCacheable(true).list();
	}
